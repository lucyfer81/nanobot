...
class MemoryFlushTrigger:
    ...
    async def trigger_flush(
        ...
        *,
        ...
        user_prompt = (
            ...
            '- "memory_entry": concise long-term facts/preferences/decisions\n'
            '- "daily_note": daily notes or rationale worth logging today\n\n'
            "Rules:\n"
        ...
        wrote = False
        if memory_entry:
            memory_store.append_long_term_entry(memory_entry)
            wrote = True
        if daily_note:
            memory_store.append_daily_note(today, daily_note)
            wrote = True
        return wrote
    ...
    def _parse_flush_payload(self, raw_text: str) -> tuple[str, str] | None:
